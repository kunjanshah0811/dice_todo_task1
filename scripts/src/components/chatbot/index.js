...
const Chatbot = () => {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50">
            ...
            <button
                className="rounded-full w-16 h-16 bg-blue-500 text-white flex items-center justify-center shadow-lg hover:bg--primary-color focus:outline-none"
                onClick={isOpen ? handleClose : handleOpen}
            >
                <img src={chatbotImage} alt="Chatbot" className="w-10 h-10 rounded-full" />
            </button>
                        
            <div className={`fixed bottom-4 right-4 z-50 bg-white rounded-lg w-full max-w-md p-4 shadow-lg ${isOpen ? 'scale-y-100' : 'scale-y-0'}`}
                 style={{
